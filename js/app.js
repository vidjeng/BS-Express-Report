          ...
          <div class="issue-body-box">
            ...
            ${iss.note ? `
              <div class="issue-note-callout">
                <strong>${notePrefix}</strong> ${this.getDisplayNote(iss.note, this.currentLang)}
              </div>
            ` : ''}
...
${divider}
🏢 *Branch:* ${this.getDisplayBranch(issue.branch, 'en')}
📅 *Date:* ${issue.dateDisplay || issue.date}
👤 *Reporter:* ${issue.reporterName} (${this.getDisplayRole(issue.position, 'en')})
${divider}
⚠️ *Issue:* ${issue.issue}
👉 *Status:* ${issue.status === 'complete' ? '✅ Resolved' : '🔴 Incomplete'}
${issue.note ? `📝 *Plan/Note:* ${this.getDisplayNote(issue.note, 'en')}\n` : ''}${divider}
📍 *HQ Operations Monitoring*` : `🚨 *ក្រុមហ៊ុនប៊ីអេស អិចប្រេស - តាមដានបញ្ហាសាខា*
${divider}
🏢 *សាខា៖* ${this.getDisplayBranch(issue.branch, 'km')}
📅 *កាលបរិច្ឆេទ៖* ${issue.dateDisplay || issue.date}
👤 *អ្នករាយការណ៍៖* ${issue.reporterName} (${this.getDisplayRole(issue.position, 'km')})
${divider}
⚠️ *បញ្ហាប្រឈម៖* ${issue.issue}
👉 *ស្ថានភាព៖* ${issue.status === 'complete' ? '✅ បានដោះស្រាយរួច' : '🔴 មិនទាន់រួចរាល់'}
${issue.note ? `📝 *ផែនការដោះស្រាយ៖* ${this.getDisplayNote(issue.note, 'km')}\n` : ''}${divider}
📍 *មជ្ឈមណ្ឌលប្រតិបត្តិការកណ្តាល (HQ)*`;
  ...
  getDisplayNote(note, lang = this.currentLang) {
    const val = String(note || '').trim();
    if (!val) return '';
    const isEn = lang === 'en';

    if (val === 'ត្រូវធ្វើនៅថ្ងៃស្អែក' || val.toLowerCase() === 'do it tomorrow') {
      return isEn ? 'Do it Tomorrow' : 'ត្រូវធ្វើនៅថ្ងៃស្អែក';
    }
    if (val === 'សកម្មភាព៖ ត្រូវធ្វើនៅថ្ងៃស្អែក' || val.toLowerCase() === 'action: do it tomorrow') {
      return isEn ? 'Action: Do it Tomorrow' : 'សកម្មភាព៖ ត្រូវធ្វើនៅថ្ងៃស្អែក';
    }
    if (val === 'ដោះស្រាយរួចរាល់' || val.toLowerCase() === 'resolved' || val.toLowerCase() === 'complete' || val.toLowerCase() === 'completed') {
      return isEn ? 'Resolved' : 'ដោះស្រាយរួចរាល់';
    }
    if (val === 'បញ្ហាមិនទាន់ដោះស្រាយពេលបិទសាខា' || val.toLowerCase() === 'unresolved issues at branch closing') {
      return isEn ? 'Unresolved issues at branch closing' : 'បញ្ហាមិនទាន់ដោះស្រាយពេលបិទសាខា';
    }
    if (val === 'Carried from previous report' || val === 'បន្តពីរបាយការណ៍មុន') {
      return isEn ? 'Carried from previous report' : 'បន្តពីរបាយការណ៍មុន';
    }
    if (isEn && val.includes('ត្រូវធ្វើនៅថ្ងៃស្អែក')) {
      return val.replace(/ត្រូវធ្វើនៅថ្ងៃស្អែក/g, 'Do it Tomorrow');
    }
    if (!isEn && val.toLowerCase().includes('do it tomorrow')) {
      return val.replace(/do it tomorrow/gi, 'ត្រូវធ្វើនៅថ្ងៃស្អែក');
    }
    return val;
  }
